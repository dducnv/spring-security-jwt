package com.example.securityspringjwt.utils;
...
public class JwtUtil {
    ...
    public static String generateTokenByAccount(Account account, int expireAfter) {
        return JWT.create()
                ...
                //.withClaim(JwtUtil.ROLE_CLAIM_KEY, user.getAuthorities().stream().map(GrantedAuthority::getAuthority).collect(Collectors.toList()))
                //when role n -> 1 user
                .withClaim(JwtUtil.ROLE_CLAIM_KEY, account.getRole()) //get first role in Authorities
                .withClaim("username", account.getEmail()) //get first role in Authorities
                .sign(getAlgorithm());
    }
}
